...
void tacc_f128_from_u32(struct tacc_f128 *f, uint32_t n) {
    ...
    f->exponent = EXP_BIAS + (31 - leading_zeroes);
}
...
static void tacc_f128_ldexp1(struct tacc_f128 *f, int exp) {
    ...
    f->exponent = EXP_BIAS + (uint16_t) exp;
...
void tacc_f128_round_f64(struct tacc_f128 *dst, struct tacc_f128 *src) {
    ...
    if (src->exponent < (EXP_BIAS - DBL_BIAS)) {
        ...
        /* TODO check */
        fully_resident_bits =
            (src->exponent + DBL_BIAS) - EXP_BIAS + DBL_SIGNIFICANT_DIGITS;
    } else {
        /* not subnormal */
    ...
    if (fully_resident_bits > 32) {
        ...
        mant_mask_discard_msb = 1 << ((32 - (64 - fully_resident_bits)) - 1);
    ...
    if (fully_resident_bits >= 32) {
        ...
        mant_mask_discard = ~mant_mask_keep;
        mant_mask_discard_msb = 1 << (32 - fully_resident_bits - 1);
